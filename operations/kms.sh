...
aws kms list-keys
aws kms encrypt --key-id <key-id/key-alias> --plaintext file://topsecret.txt --output text --query CiphertextBlob
aws kms encrypt --key-id <key-id/key-alias> --plaintext file://topsecret.txt --output text --query CiphertextBlob > topsecret.base64.encrypted
aws kms encrypt --key-id <key-id/key-alias> --plaintext file://topsecret.txt --output text --query CiphertextBlob | base64 --decode > topsecret.encrypted
...
aws kms generate-data-key --key-id <key-id/key-alias> --key-spec AES_256
